...
# session state needs to come before the css block or dark_mode won't exist yet
defaults = {"authenticated": False, "report_generated": False, "logs": [], "dark_mode": True}
for key, val in defaults.items():
    if key not in st.session_state:
        st.session_state[key] = val

# theme colours - put them all in one dict so i only have to change things once
...
# IBM Plex because it looks more professional/government-ish than the default fonts
st.markdown(f"""
<style>
...
# lookup dicts - codes from the STATS19 documentation
DAYS = {
...
# short descriptions of why each condition is risky - used in the report text
RISK_NARRATIVE = {
...
def load_data(path):
    ...
    # parse time string into an hour int so we can filter by it
...
# cache_resource keeps the model alive between reruns so it doesn't retrain every time
...
# dark mode toggle - rebuilds the whole colour dict on rerun
...
if access_mode == "Admin Portal" and not st.session_state["authenticated"]:
    ...
    if st.sidebar.button("Verify Identity"):
        # TODO: move to st.secrets before submission
        if pwd == "IT_Admin_2026":
...
# filter the dataset down to matching rows
filtered = data[
...
severity_map = {
    1: ("Low Risk  ·  Slight",       "badge-low"),
    2: ("Elevated Risk  ·  Serious", "badge-medium"),
    3: ("Critical Risk  ·  Fatal",   "badge-high")
}
...
with left:
    ...
    if st.session_state["report_generated"]:
        if not filtered.empty:
            ...
            slight_n  = len(filtered[filtered["collision_severity"] == 1])
            serious_n = len(filtered[filtered["collision_severity"] == 2])
            fatal_n   = len(filtered[filtered["collision_severity"] == 3])
...
# regional breakdown - only shows if there's data to display
if not filtered.empty:
    st.markdown("---")
    ...
    # check if the hours around the selection are busier - helps with deployment timing
    ...
    with dep3:
        ...
        fatal_pct   = len(filtered[filtered["collision_severity"] == 3]) / len(filtered) * 100
